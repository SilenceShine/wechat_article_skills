...
| **乱码检查** | 使用 Grep 搜索 `��` 或 `\ufffd` | 乱码 → 必须修复后才能发布 |

**⚠️ 乱码检查（必须执行）**：

由于编码问题，生成的 HTML 文件可能包含乱码字符（如 `��`）。这是 UTF-8 多字节中文字符被截断导致的。
...
| **发现乱码字符 `��`** | UTF-8 编码截断 | **必须修复**：使用 Grep 定位 → Edit 修复 → 再次检查 |
